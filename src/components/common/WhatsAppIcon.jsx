 ...
 *
 * Inherits `currentColor` like every other icon in the system, so it takes the
 * row's own colour and only picks up WhatsApp green where a rule opts into it.
 * Sized and stroked to sit on the same optical weight as the lucide set.
 */
export default function WhatsAppIcon({ size = 16, className = "", title }) {
  return (
    ...
    >
      ...
      <path d="M8.53 7.33c-.18-.4-.37-.41-.54-.42h-.46c-.16 0-.42.06-.64.3-.22.24-.84.82-.84 2s.86 2.32.98 2.48c.12.16 1.67 2.67 4.12 3.64 2.04.8 2.45.64 2.89.6.44-.04 1.42-.58 1.62-1.14.2-.56.2-1.04.14-1.14-.06-.1-.22-.16-.46-.28-.24-.12-1.42-.7-1.64-.78-.22-.08-.38-.12-.54.12-.16.24-.62.78-.76.94-.14.16-.28.18-.52.06-.24-.12-1.01-.37-1.93-1.19-.71-.63-1.19-1.42-1.33-1.66-.14-.24-.02-.37.1-.49.11-.11.24-.28.36-.42.12-.14.16-.24.24-.4.08-.16.04-.3-.02-.42-.06-.12-.53-1.3-.74-1.8Z" />
    </svg>
  );
}

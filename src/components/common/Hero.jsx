      ...
      <div className="container product-hero-inner">
        <Reveal className="product-hero-copy">
          {/* Opt-in, defaults to nothing: only a caller that explicitly
              ...
              Luxe) is byte-for-byte unaffected. A sibling of the text stack
              (not interleaved with it) so a two-column layout can place it
              beside the whole stack without the multi-row-span grid sizing
              quirk that comes from spanning one item across several
              independently auto-sized rows. */}
          {topAccessory}

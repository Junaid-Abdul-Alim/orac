...
2. **Expansion** — the single frame separates into three (International / Eventus / Luxe). Each mini-frame shows a still image, the venture name, and nothing else. This replaces today's `CompanyPortfolio` card grid in position, but not necessarily in component — `BusinessCard` can evolve into this rather than being replaced outright (see §11).
...
- **Venture indicators:** the existing `BusinessSwitcher` dropdown is sufficient functionally; it needs the same per-venture accent treatment as the nav, applied to its three entries.
...
- **Footer:** keep structurally; resolve the Businesses/Companies/ventures naming (pending decision) before or during this milestone.
...
**Note on "ORAC Evolution":** an earlier draft of this spec carried a 4th-world placeholder for a venture called "ORAC Evolution." Confirmed by the user to be a mistake — it does not exist and is not planned. The homepage opening, expansion, and every venture-world decision in this spec are designed for exactly **three** worlds, with no reserved fourth slot.

**Connective tissue across all three:** the frame's position and entry animation, the shared nav, the shared typography system, and the gold-as-accent (not gold-as-strategy) rule apply identically everywhere. Nothing about a venture's distinct treatment should require a different component library, a different motion system, or a different container width.
...
- **Venture-specific accents:** introduce one additional accent variable per venture (`--accent-international`, `--accent-eventus`, `--accent-luxe`), used sparingly — a rule line, a hover state, the nav indicator (§3) — never as a full background recolour of a venture page. This directly answers the brief's "gold may remain a controlled accent but must not be the entire luxury strategy" requirement by giving each world a second, quieter signal beyond gold.
- **Role of gold:** stays as the shared ORAC accent (rules, eyebrows, focus states) across all three worlds — it signals "this is ORAC," while the new per-venture accent signals "this is which world."
...
- **Background transitions:** the frame-to-three-frames moment (§2 stage 2) is the one place a background shift should be deliberate and slow; elsewhere, keep the current cream/white/ink system.
...
- **Hover behaviour:** keep existing button/link/card hover states; the frame device may gain a subtle hover state on the three-frames moment (stage 2) to invite exploration, using the same transform-based hover language already present in `.maison-card:hover`, `.edition-card:hover`, etc.
...
- **Mobile simplification:** the three-frames separation (stage 2) should simplify to a simple stacked reveal on narrow viewports rather than attempting the same spatial transform — consistent with the brief's "recompose, don't shrink" rule.
...
- **1440px (desktop):** full expression — frame device, three-frame separation, side-by-side chapter compositions.
- **1024px (tablet landscape):** frame device retained; three-frame separation stays a single row with tighter spacing (three items fit comfortably at this width, unlike a four-item grid); chapter compositions that are two-column at 1440 may need to narrow their text column (existing `--max-narrow` token already supports this).
- **768px (tablet/mobile boundary):** audit confirms the existing nav already switches to the mobile menu here; the three-frame moment should become a vertical stack at or before this width; chapter compositions become single-column.
- **390px (mobile):** the true "recomposed" case per the brief — e.g. the opening frame may drop its separation animation entirely in favour of a simple sequential reveal of the three venture names (still inside a frame-shaped container for consistency, but without the spatial transform). Every touch target must remain comfortably sized (audit found no current violations to fix, only new components to hold to the same standard).
...
- Keyboard access: the new three-frame moment and any per-venture nav accent must remain fully keyboard-operable — test with the same Tab/Escape/Arrow patterns already implemented in `Navbar.jsx`.
...
- **Objective:** build the three-frames expansion and three distinct venture chapters, replacing `CompanyPortfolio` + `InternationalPreview` + `EventusPreview` + `LuxePreview`.
...
- **Risks:** medium-high — the largest single markup change in this plan.
...
### Milestone 8 — Responsive and accessibility pass
- **Objective:** dedicated verification pass across everything built in Milestones 1–7, at all 4 required breakpoints, plus the accessibility requirements in §10.
...
### Milestone 9 — Performance and final polish
...
- **Must remain unchanged:** no visual regression from the approved design in Milestones 1–8 — this milestone optimises delivery, not appearance.
...
- **Decision:** whether Apple/Bakinatajna remain active supplementary references ([Decisions](ORAC-DECISIONS.md) item 2) — needed before Milestone 1 sets final tone/spacing targets, though it does not block starting.
- **Decision:** "Businesses" vs. "Companies" vs. "ventures" terminology ([Decisions](ORAC-DECISIONS.md) item 3) — needed before Milestone 2 (nav copy).
